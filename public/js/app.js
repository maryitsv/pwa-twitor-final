...
let camara = new Camara($('#player')[0]);
...
// mostrar la notificacion de subscribirse
notificarme();
...
btnLocation.on('click', () => {
    ...
    if(navigator.geolocation){
        console.log('si soporta geolocation');
        const geo = navigator.geolocation.getCurrentPosition((coordenadas)=>{
            console.log('si encontro',coordenadas);
            mostrarMapaModal(coordenadas.coords.latitude, coordenadas.coords.longitude);
...
timeline.on('click','li', function(){
    ...
    const longitud = $(this).data('lng');
    const latitud = $(this).data('lat');

    const mesj = {
        title:user,
        text:mensaje,
        urls:`https://www.google.com/maps/@${latitud},${longitud},152`
    };
    console.log(mesj);
    navigator.share(mesj).then(()=>{console.log('yupi shared')});
